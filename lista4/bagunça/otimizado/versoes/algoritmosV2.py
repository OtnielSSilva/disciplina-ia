        ...
        if no.estado not in explorados or custo_atual < explorados[no.estado]:
            ...
            for sucessor in expandir(no, problema):
                ...
                if (estado_sucessor not in explorados or custo_sucessor < explorados.get(estado_sucessor, float('inf'))):
                    if (estado_sucessor not in estados_na_borda or custo_sucessor < estados_na_borda[estado_sucessor]):
                        heapq.heappush(borda, (custo_sucessor, sucessor))
                        estados_na_borda[estado_sucessor] = custo_sucessor
                        todos_nos_na_borda.append(estado_sucessor)
...
# Busca em Profundidade: Implementação de uma limitação de profundidade para reduzir a BORDA.
# Busca de Custo Uniforme: Evitar inserir estados na BORDA se já existir um caminho de menor custo conhecido para aquele estado.
