...
class Problema:
    ...
    def expandir(self, estado):
        # Retorna tanto o estado quanto o custo da aresta
...
def busca_em_profundidade(problema, output_text):
    # Pilha para a busca em profundidade
    borda = deque([problema.estado_inicial])
    explorado = set()
    caminho = {problema.estado_inicial: None}  # Para reconstruir o caminho
    passos = 0
...
def busca_de_custo_uniforme(problema, output_text):
    borda = []
    # Fila de prioridade para a busca de custo uniforme
    heapq.heappush(borda, (0, problema.estado_inicial))
    explorado = set()
    caminho = {problema.estado_inicial: None}  # Para reconstruir o caminho
    ...
    while borda:
        ...
        output_text.insert(
            tk.END, f"Passo {passos}: Explorando {no} com custo {custo_atual}\n")
        output_text.insert(
            tk.END, f"Borda atual: {[(c, n) for c, n in borda]}\n\n")
        output_text.update_idletasks()  # Atualizar a interface imediatamente

        if problema.objetivo_alcancado(no):
            output_text.insert(
                tk.END, f"Custo total da solução: {custo_atual}\n")
            output_text.insert(tk.END, f"Nós expandidos: {len(explorado)}\n")
            output_text.update_idletasks()  # Atualizar a interface imediatamente
            return reconstruir_caminho(caminho, problema.estado_inicial, no, explorado, borda, output_text)
...
def iniciar_busca():
    ...
    if not estado_inicial or not estado_objetivo:
        output_text.insert(
            tk.END, "Erro: Por favor, selecione o nó inicial e o nó objetivo.\n")
        return

    if estado_inicial not in nos_validos or estado_objetivo not in nos_validos:
        output_text.insert(
            tk.END, "Erro: Nó inicial ou objetivo inválido para o grafo selecionado.\n")
        return

    problema = Problema(estado_inicial=estado_inicial,
                        estado_objetivo=estado_objetivo, grafo=grafo)
...
grafo_romenia_rb = tk.Radiobutton(
    frame_grafo, text="Romênia", variable=grafo_var, value="Romênia", command=atualizar_nos)
grafo_personalizado_rb = tk.Radiobutton(
    frame_grafo, text="Personalizado", variable=grafo_var, value="Personalizado", command=atualizar_nos)
...
busca_largura_rb = tk.Radiobutton(
    frame_busca, text="Largura", variable=busca_var, value="Largura")
busca_profundidade_rb = tk.Radiobutton(
    frame_busca, text="Profundidade", variable=busca_var, value="Profundidade")
busca_custo_uniforme_rb = tk.Radiobutton(
    frame_busca, text="Custo Uniforme", variable=busca_var, value="Custo Uniforme")
...
tk.Label(frame_entrada, text="Nó Inicial:").grid(
...
tk.Label(frame_entrada, text="Nó Objetivo:").grid(
...
output_text = tk.Text(frame_output, height=20, width=80,
